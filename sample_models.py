...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        ...
        deep_rnn = GRU(units, activation='relu',
            return_sequences=True, implementation=2, name=rnn_name)(cur_input)
        deep_rnn = BatchNormalization(name=bn_name)(deep_rnn)
...
#     bn_cnn_2 = BatchNormalization(name='bn_conv_2d')(conv_1d_2)
    ...
    for i in range(recur_layers):
        ...
        if i == 0:
            ...
            cur_input = deep_rnn
        deep_rnn = Bidirectional(GRU(units, activation='relu',
            return_sequences=True, implementation=2, name=rnn_name, dropout=0.15, recurrent_dropout=0.15), merge_mode='concat')(cur_input)
        deep_rnn = BatchNormalization(name=bn_name)(deep_rnn)
    # TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(deep_rnn)
